...
| options.onRequestError | <code>function</code> | Callback for when requests are received. |
| options.onRequestError | <code>function</code> | Callback for when requested methods throw errors. |
| options.onServerError | <code>function</code> | Callback for server errors. |
...
invalid port number the promise will be rejected with an [Error](Error).
...
the promise will be rejected with an [Error](Error).

C:\Users\Daniel\Documents\GitHub\http-jsonrpc-server\lib>jsdoc2md http-jsonrpc-server.js
<a name="RpcServer"></a>
